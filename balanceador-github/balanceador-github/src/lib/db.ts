...
// Credenciais armazenadas de forma segura (não visíveis no front-end)
const SECURE_USERNAME = process.env.SECURE_USERNAME || 'RicardoToldo';
const SECURE_PASSWORD = process.env.SECURE_PASSWORD || 'ricardo1008';
        ...
        first: async <T>() => {
          ...
          if (query.includes('SELECT * FROM users WHERE username = ? AND password = ?')) {
            ...
            if (username === SECURE_USERNAME && password === SECURE_PASSWORD) {
              return { id: 1, username: SECURE_USERNAME, created_at: new Date().toISOString() } as T;
            }
            return null;
          }
          return null;
